...
class HfSanitizerCoveragePass
    : public llvm::PassInfoMixin<HfSanitizerCoveragePass> {
public:
  explicit HfSanitizerCoveragePass(llvm::SanitizerCoverageOptions Options)
      : Options(Options) {}
...
/// firedancer-io/honggfuzz START
PreservedAnalyses hfuzz::HfSanitizerCoveragePass::run(Module &M,
  ...
  ModuleSanitizerCoverage ModuleSancov(M, DTCallback, PDTCallback, Options,
                                       nullptr, nullptr);
...
/// firedancer-io/honggfuzz START
      C->emitError("gated callbacks are only supported with trace-pc-guard or trace-cmp");
/// firedancer-io/honggfuzz END
      return true;
    }
...
/// firedancer-io/honggfuzz START
    auto Inc = IRB.CreateBinaryIntrinsic(Intrinsic::uadd_sat, Load,
...
void checkHostLlvmVersion() {
  using GetVersionFn = void (*)(unsigned *, unsigned *, unsigned *);
  auto fn = reinterpret_cast<GetVersionFn>(dlsym(RTLD_DEFAULT, "LLVMGetVersion"));
  if (!fn)
    return;
  ...
  if (Major != LLVM_VERSION_MAJOR || Minor != LLVM_VERSION_MINOR ||
    ...
    errs() << "hfuzz sancov plugin: built against LLVM " << LLVM_VERSION_STRING
           << " but loaded into LLVM " << Major << "." << Minor << "." << Patch
           << "\n";
    errs().flush();
    _exit(1);
  }
...
extern "C" LLVM_ATTRIBUTE_WEAK PassPluginLibraryInfo llvmGetPassPluginInfo() {
  checkHostLlvmVersion();
  return {LLVM_PLUGIN_API_VERSION, "hfuzz-sancov", LLVM_VERSION_STRING,
          [](PassBuilder &PB) {
            PB.registerOptimizerLastEPCallback(
                [](ModulePassManager &MPM, OptimizationLevel,
                   ThinOrFullLTOPhase) {
                  MPM.addPass(hfuzz::HfSanitizerCoveragePass(options()));
                });
          }};
}
/// firedancer-io/honggfuzz END
