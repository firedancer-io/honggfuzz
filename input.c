...
bool input_prepareDynamicInput(run_t* run, bool needs_mangle) {
            ...
            if (logNow - lastLog >= 150) {
                ...
                if (total > 0) {
                    ...
                    uint64_t childTruncated = 0;
                    {
                        ...
                        uint64_t elfFixupOk = 0, execFail = 0, verifyCalls = 0, harnessReject = 0;
                        if (cov) {
                        ...
                        LOG_I("[MUTATION-HEALTH] proto_parse=%zu/%zu (%.1f%%) custom_mutator=%zu/%zu"
                              ...
                              " %s"
                              " elf_ok=%zu exec_fail=%zu verify=%zu harness_reject=%zu",
                              ...
                              kindsBuf,
                              (size_t)elfFixupOk, (size_t)execFail, (size_t)verifyCalls, (size_t)harnessReject);
                    }
    ...
    if (run->pendingStatsLog) {
        ...
        if (cov) {
            ...
            uint64_t elfOk = 0, execFail = 0, verify = 0, harnessReject = 0;
            ...
            if (ppCalls > 0 || cmCalls > 0) {
                ...
                hfuzz_metrics_log_mutation_health(s->mutationsCnt,
                                                  ...
                                                  elfOk, execFail, verify, harnessReject);
            }
        }
    }
